...
/* ==================== 9 — DATA QUALITY ANALYSIS ==================== */
{
  ...
  s.addImage({ path: path.join(IMG, "missing_values.png"), x: M, y: 1.5, w: 5.9, h: 2.95 });
  s.addImage({ path: path.join(IMG, "outliers_boxplots.png"), x: M, y: 4.52, w: 5.9, h: 2.36 });
...
/* ================ 10 — EDA: DISTRIBUTION AND AREA ================ */
{
  ...
  s.addImage({ path: path.join(IMG, "price_distribution.png"), x: M, y: 1.5, w: 6.0, h: 3.48 });
  s.addImage({ path: path.join(IMG, "area_vs_price.png"), x: 6.86, y: 1.5, w: 5.82, h: 3.48 });
...
/* ============= 11 — EDA: CONFIGURATION AND LOCATION ============= */
{
  ...
  s.addImage({ path: path.join(IMG, "bhk_vs_price.png"), x: M, y: 1.46, w: 6.0, h: 3.66 });
  s.addImage({ path: path.join(IMG, "locality_median_price.png"), x: 6.86, y: 1.46, w: 5.82, h: 3.66 });
  ...
  s.addText("Configuration", { x: M + 0.22, y: 5.4, w: 5.5, h: 0.26,
    fontFace: BODY, fontSize: 12.5, bold: true, color: NAVY, margin: 0 });
  s.addText(
    `Median price rises consistently with configuration:\n${bhkLine}.\nThe spread within each group is wide, so configuration alone does not determine price.`,
...
/* ===================== 12 — CORRELATION ANALYSIS ===================== */
{
  ...
  s.addImage({ path: path.join(IMG, "correlation_heatmap.png"), x: M, y: 1.62, w: 5.5, h: 4.62 });
  ...
  tbl(s, rows, { x: 6.42, y: 1.62, w: 6.26, colW: [3.9, 2.36], rowH: 0.36 });

  s.addText("Interpretation", { x: 6.42, y: 3.62, w: 6.26, h: 0.3,
    fontFace: HEAD, fontSize: 15, bold: true, color: NAVY, margin: 0 });
  s.addText(
    [
    ...
    { x: 6.42, y: 4.02, w: 6.26, h: 2.3, fontFace: BODY, fontSize: 12,
...
const OUT = path.join(ROOT, "Project_Review_1_Bengaluru_Property_ML.pptx");
